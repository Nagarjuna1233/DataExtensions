...
public class AnutaApi {

    private static final Logger LOG = LoggerFactory.getLogger(AnutaApi.class);
    
    private String userId;
    private String password;
    private String baseUrl;

    public <T> T anutaGetCall(String url, Map<String, String> paramKeyValues, final Class<T> responseType) throws ResoucePullingFailedException
             {
        try {
            return new AvalonJerseyClient.AvaloneHttpsBuilder(
                    AvalonUtils.buildPathParamUrl(this.baseUrl + url, paramKeyValues)).setAccept(MediaType.APPLICATION_JSON)
                            ...
                            .build().get(responseType,"ANUTA" ,AvalonUtils.buildPathParamUrl(this.baseUrl + url, paramKeyValues),"Error While Pulling");
        } catch (Exception e) {
