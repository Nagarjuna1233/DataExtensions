...
public class PrimeDeviceQosAppResolver implements MethodResolver {

    static final String METHOD_NAME = "determinieAppTypeForDeviceQOSPolicy";
    private CanonicalItemService canonicalItemService;
    private PrimeDeviceQosAppEcecutor executor;

    @Override
    public MethodExecutor resolve(EvaluationContext arg0, Object arg1, String name, List<TypeDescriptor> argumentTypes)
            throws AccessException {
        executor = new PrimeDeviceQosAppEcecutor(canonicalItemService);
    ...
    private static class PrimeDeviceQosAppEcecutor implements MethodExecutor {

        private CanonicalItemService canonicalItemService;

        private static final String ITEM_TYPE = "CanonicalPrimeApplication";

        public PrimeDeviceQosAppEcecutor(CanonicalItemService canonicalItemService) {
        ...
        private static String getAppType(List<CanonicalItem> intSize, String input) {
            String result = "";
            for (CanonicalItem colItem : intSize) {
                String val = (String) colItem.getField("applicationName");
                if (val.equalsIgnoreCase(input)) {
                    result = (String) colItem.getField("appType");
                    intSize.clear();
                    break;
                }
            }
