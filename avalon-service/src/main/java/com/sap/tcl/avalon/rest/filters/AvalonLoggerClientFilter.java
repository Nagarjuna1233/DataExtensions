package com.sap.tcl.avalon.rest.filters;
...
/**
 * 
 * @author Techouts-1194 Logging filter for rest client
 *         {@link AvalonJerseyClient}
 */
public class AvalonLoggerClientFilter implements ClientResponseFilter, ClientRequestFilter {
    ...
    private static void requestLog(URI uri, MultivaluedMap<String, ?> headers) {
        LOG.info("Request URI: {}", uri.getPath());
        LOG.info("Headers: ");
        headers.entrySet().forEach(h -> LOG.info(h.getKey() + ": " + h.getValue()));
    }

    private static void responseLog(URI uri, MultivaluedMap<String, ?> headers, int code) {
        LOG.info("Request URI: {}", uri.getPath());
